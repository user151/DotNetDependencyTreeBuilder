...
public class CSharpProjectParserTests : IDisposable
{
    private readonly string _tempDirectory;
    private readonly CSharpProjectParser _parser;

    public CSharpProjectParserTests()
    {
        _tempDirectory = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString());
        Directory.CreateDirectory(_tempDirectory);
        _parser = new CSharpProjectParser();
    }
    ...
    [Fact]
    public void CanParse_WithCsprojFile_ReturnsTrue()
    {
        // Arrange
    ...
    [Fact]
    public void CanParse_WithVbprojFile_ReturnsFalse()
    {
        // Arrange
    ...
    {
        // Arrange
        var filePath = Path.Combine(_tempDirectory, "nonexistent.csproj");
    ...
    public async Task ParseProjectFileAsync_WithSimpleProject_ReturnsBasicProjectInfo()
    {
        ...
        var projectContent = """
            ...
            """;
        var filePath = Path.Combine(_tempDirectory, "SimpleProject.csproj");
        ...
        result.Type.Should().Be(ProjectType.CSharp);
    ...
    public async Task ParseProjectFileAsync_WithProjectReferences_ExtractsProjectDependencies()
    {
        ...
        var projectContent = """
            ...
            """;
        var filePath = Path.Combine(_tempDirectory, "ProjectWithReferences.csproj");
        await File.WriteAllTextAsync(filePath, projectContent);
        ...
        result.ProjectReferences[0].ReferencedProjectPath.Should().Be(@"..\Core.Library\Core.Library.csproj");
        ...
        result.ProjectReferences[1].ReferencedProjectPath.Should().Be(@"..\Utilities\Utilities.csproj");
    ...
    public async Task ParseProjectFileAsync_WithPackageReferences_ExtractsPackageDependencies()
    {
        ...
        var projectContent = """
            ...
            """;
        var filePath = Path.Combine(_tempDirectory, "ProjectWithPackages.csproj");
        await File.WriteAllTextAsync(filePath, projectContent);
    ...
    public async Task ParseProjectFileAsync_WithMultipleTargetFrameworks_ExtractsTargetFrameworks()
    {
        ...
        var projectContent = """
            ...
            """;
        var filePath = Path.Combine(_tempDirectory, "MultiTargetProject.csproj");
        await File.WriteAllTextAsync(filePath, projectContent);
    ...
    public async Task ParseProjectFileAsync_WithComplexProject_ExtractsAllInformation()
    {
        ...
        var projectContent = """
            ...
            """;
        var filePath = Path.Combine(_tempDirectory, "ComplexProject.csproj");
        await File.WriteAllTextAsync(filePath, projectContent);
    ...
    public async Task ParseProjectFileAsync_WithMalformedXml_ThrowsProjectParsingException()
    {
        ...
        var projectContent = """
            <Project Sdk="Microsoft.NET.Sdk">
              ...
              <!-- Missing closing tag -->
            """;
        var filePath = Path.Combine(_tempDirectory, "MalformedProject.csproj");
        await File.WriteAllTextAsync(filePath, projectContent);
    ...
    {
        // Arrange
        var filePath = Path.Combine(_tempDirectory, "EmptyProject.csproj");
    ...
    public async Task ParseProjectFileAsync_WithNoTargetFramework_ReturnsEmptyTargetFramework()
    {
        ...
        var projectContent = """
            ...
            """;
        var filePath = Path.Combine(_tempDirectory, "NoTargetFramework.csproj");
        await File.WriteAllTextAsync(filePath, projectContent);
