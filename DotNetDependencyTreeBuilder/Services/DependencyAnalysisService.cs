...
public class DependencyAnalysisService : IDependencyAnalysisService
{
   ...
   public BuildOrder GenerateBuildOrder(DependencyGraph dependencyGraph)
   {
      ...
      try
      {
         //sort dependencyGraph.AdjacencyList by value .Count
         var adjacencyListOrdered = dependencyGraph.AdjacencyList.OrderBy(kvp => kvp.Value.Count).ToDictionary();
         dependencyGraph.AdjacencyList = adjacencyListOrdered;
