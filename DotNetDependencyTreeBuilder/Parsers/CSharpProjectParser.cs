...
/// <summary>
/// Parser for C# project files (.csproj)
/// </summary>
public class CSharpProjectParser : IProjectFileParser
{
   /// <summary>
   /// Determines if this parser can handle the specified file type
   /// </summary>
   /// <param name="filePath">Path to the project file</param>
   /// <returns>True if this parser can handle .csproj files</returns>
   ...
   /// <summary>
   /// Parses a C# project file to extract project information and dependencies
   /// </summary>
         ...
         if (assemblyReferenceNodes != null && assemblyReferenceNodes.Count > 0)
         {
            foreach (XmlNode node in assemblyReferenceNodes)
            {
               ...
               if (includeAttribute != null)
               {
                  ...
                  if (CouldBeReferenceByName(assemblyName))
                  {
                     var dependency = new ProjectDependency
                     {
   ...
   /// <summary>
   /// Determines if an assembly name could potentially be a project reference
   /// </summary>
   /// <param name="assemblyName"></param>
   /// <returns></returns>
   private static bool CouldBeReferenceByName(string assemblyName)
   {
      if (string.IsNullOrWhiteSpace(assemblyName))
         return false;

      // Skip obvious system/framework assemblies
      var systemPrefixes = new[] { "Microsoft", "System", "mscorlib", "netstandard", "Windows", "PresentationFramework", "PresentationCore"};
